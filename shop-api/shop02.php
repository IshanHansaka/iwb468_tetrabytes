...
switch ($method) {
    case 'GET':
        if (isset($_GET['id'])) {
            $id = $_GET['id'];
            $stmt = $conn->prepare("SELECT * FROM shop02 WHERE id = ?");
    ...
    case 'POST':
        if (isset($input['model_name'], $input['price'], $input['warranty'], $input['in_stock'])) {
            ...
            $stmt = $conn->prepare("INSERT INTO shop02 (model_name, price, warranty, in_stock) VALUES (?, ?, ?, ?)");
    ...
    case 'DELETE':
        if (isset($_GET['id'])) {
            $id = $_GET['id'];
            $stmt = $conn->prepare("DELETE FROM shop02 WHERE id = ?");
            $stmt->bind_param("i", $id);
            $stmt->execute();
